...
function OnFailureCustomer() {
    ...
    $("#modal-insert-league").modal("hide");
    $("#modal-edit-league").modal("hide");
    $('form').trigger("reset");
    CKupdate();
}
...
function createLeague() {
    ...
    if ($("table#tableDictionary").find('tr.oneselected').length === 0) {
        ...
        $.ajax({
            ...
            dataType: "html",
            success: function (data) {
                $("#modal-insert-league").modal("show");
                $("#insert-league").select2();
                $("#insert-card").select2();
            }
        })
    }
...
function updateLeague(id) {
    ...
    $.ajax({
        type: "POST",
        url: '/CustomerOtherTab/EditLeague',
        data: JSON.stringify(dataPost),
        contentType: "application/json; charset=utf-8",
        dataType: "html",
        success: function (data) {
            $("#info-data-league").html(data);
            $("#edit-league").select2();
            $("#edit-card").select2();
            $("#modal-edit-league").modal("show");
        }
    })
}
...
function deleteLeague(id) {
    if (confirm('Bạn thực sự muốn xóa mục này ?')) {
        ...
        $.ajax({
            type: "POST",
            url: '/CustomerOtherTab/DeleteLeague',
            data: JSON.stringify(dataPost),
            contentType: "application/json; charset=utf-8",
            dataType: "html",
            success: function (data) {
                alert("Xóa dữ liệu thành công!!!");
                $("#thelienminh").html(data);
            }
        });
    }
}