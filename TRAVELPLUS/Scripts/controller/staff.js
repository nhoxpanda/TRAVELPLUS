...
$("#insert-typeID").select2();
$("#insert-contracttype").select2();
...
$("#btnEdit").click(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#edit-citizenship").select2();
            $("#edit-typeID").select2();

            /***** hinh anh *****/
            $("#edit-Image").change(function () {
                var data = new FormData();
                data.append('Image', $('#edit-Image')[0].files[0]);
                var ajaxRequest = $.ajax({
                    type: "POST",
                    url: 'StaffManage/UploadImage',
                    contentType: false,
                    processData: false,
                    data: data
                });
            ...
            /***** cập nhật lương chính thức *****/
            CalculatorSalary('Edit');
...
/*** khen thưởng/kỷ luật ***/
function OnFailureBonusDiscipline() {
    alert("Lỗi!");
    $("#modal-edit-bonusdiscipline").modal("hide");
    $("#modal-insert-bonusdiscipline").modal("hide");
}
function OnSuccessBonusDiscipline() {
    alert("Đã lưu!");
    $("#modal-edit-bonusdiscipline").modal("hide");
    $("#modal-insert-bonusdiscipline").modal("hide");
}
...
function CalculatorSalary(txt) {
    var coban = parseFloat($("#txt" + txt + "BasicSalary").val() != "" ? $("#txt" + txt + "BasicSalary").val() : 0);
    var phucap = parseFloat($("#txt" + txt + "SubsidySalary").val() != "" ? $("#txt" + txt + "SubsidySalary").val() : 0);
    var trocap = parseFloat($("#txt" + txt + "AllowanceSalary").val() != "" ? $("#txt" + txt + "AllowanceSalary").val() : 0);
    var chinhthuc = coban + phucap + trocap;
    $("#txt" + txt + "CurSalary").val(checkMoney(chinhthuc));
}