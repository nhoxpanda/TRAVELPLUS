...
namespace TRAVELPLUS.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        #region Chọn quốc gia -> tỉnh thành -> sân bay

        public JsonResult ProvinceList(int id)
        {
           
            var items = _tagsRepository.GetAllAsQueryable().AsEnumerable()
                .Where(p => _tagsRepository.GetAllAsQueryable().FirstOrDefault(x => x.ParentId == id).ParentId == id && p.TypeTag == 5 && p.IsDelete == false).ToList();
            return Json(new SelectList(items, "Id", "Tag"), JsonRequestBehavior.AllowGet);
        }

        public JsonResult AirportList(int id)
        {
            var items = _airportRepository.GetAllAsQueryable().Where(p => p.ProvinceId == id).ToList();
            return Json(new SelectList(items, "Id", "Code"), JsonRequestBehavior.AllowGet);
        }
