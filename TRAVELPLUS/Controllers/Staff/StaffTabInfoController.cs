...
namespace TRAVELPLUS.Controllers.Staff
{
    [Authorize]
    public class StaffTabInfoController : BaseController
    {
        ...
        public ActionResult InfoNgayNghiPhep(int id)
        {
            ...
            var model = _staffDayOffRepository.GetAllAsQueryable().AsEnumerable()
                .Where(c => c.StaffId == id && c.StartDay.Year == DateTime.Now.Year)
                .OrderByDescending(p => p.StartDay).ToList();
            ViewBag.DayOffLastYear = _staffDayOffRepository.GetAllAsQueryable().AsEnumerable()
                .Where(c => c.StaffId == id && c.StartDay.Year == DateTime.Now.Year - 1)
                .OrderByDescending(p => p.StartDay).ToList();
            ViewBag.Staff = _staffRepository.FindId(id);
