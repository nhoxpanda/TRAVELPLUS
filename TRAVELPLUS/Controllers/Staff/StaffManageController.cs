...
namespace TRAVELPLUS.Controllers
{
    ...
    public class StaffManageController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            var model = _staffRepository.GetAllAsQueryable().AsEnumerable()
                    ...
                    .Select(p => new StaffListViewModel
                    {
                        ...
                        OfficialSalary = p.tbl_StaffSalary.LastOrDefault() != null ? string.Format("{0:0,0}", p.tbl_StaffSalary.LastOrDefault().CurSalary).Replace(",", ".") : "",
                        ...
                        Citizenship = p.tbl_TagsCitizenship != null ? p.tbl_TagsCitizenship.Tag : "",
                        Headquarter = p.HeadquarterId != null ? _headquaterRepository.FindId(p.HeadquarterId).ShortName : "",
                        ContractEndDate = p.ContractEndDate != null ? p.ContractEndDate.Value.ToString("dd-MM-yyyy") : "",
                        ContractFromDate = p.ContractFromDate != null ? p.ContractFromDate.Value.ToString("dd-MM-yyyy") : "",
                        Language = p.Language,
                        ContractType = p.tbl_DictionaryContractType != null ? p.tbl_DictionaryContractType.Name : ""
                    }).ToList();
            return View(model);
        }
        ...
        #region Create
        [HttpPost]
        public async Task<ActionResult> Create(StaffViewModel model, FormCollection form, HttpPostedFileBase Image, IEnumerable<HttpPostedFileBase> FileContractStaff)
        {
            Permission(clsPermission.GetUser().PermissionID, 6);
            try
            {
                ...
                if (model.SingleStaff.EndWork != null)
                {
                    model.SingleStaff.IsLock = true;
                }
                else
                {
                    model.SingleStaff.IsLock = false;
                ...
                if (FileContractStaff != null)
                {
                    foreach (var file in FileContractStaff)
                    {
                        if (file != null && file.ContentLength > 0)
                        {
                            String pathFile = Server.MapPath("~/Upload/file/" + file.FileName);
                            ...
                            var doc = new tbl_DocumentFile()
                            {
                                ...
                                FileSize = Common.ConvertFileSize(file.ContentLength),
                                DictionaryId = 3345
                            };
                            if (await _documentFileRepository.Create(doc))
                            {
                                Session["StaffFile"] = null;
                                UpdateHistory.SaveHistory(60, "Thêm mới tài liệu, code: " + doc.Code,
                                    null, //appointment
                                    null, //contract
                                    null, //customer
                                    null, //partner
                ...
                if (model.ContractFromDate != null && model.ContractFromDate.Value.Year >= 1980)
                {
                    model.SingleStaff.ContractFromDate = model.ContractFromDate;
                }
                if (model.ContractEndDate != null && model.ContractEndDate.Value.Year >= 1980)
                {
                    model.SingleStaff.ContractEndDate = model.ContractEndDate;
                }
                ...
                if (await _staffRepository.Create(model.SingleStaff))
                {
                    ...
                    {
                        StaffId = model.SingleStaff.Id,
                        BasicSalary = model.SingleStaffSalary.BasicSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.BasicSalary"].Replace(".", "")) : 0,
                        SubsidySalary = model.SingleStaffSalary.SubsidySalary != null ? Convert.ToDecimal(form["SingleStaffSalary.SubsidySalary"].Replace(".", "")) : 0,
                        AllowanceSalary = model.SingleStaffSalary.AllowanceSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.AllowanceSalary"].Replace(".", "")) : 0,
                        CurSalary = model.SingleStaffSalary.CurSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.CurSalary"].Replace(".", "")) : 0,
                        StartSalary = model.SingleStaffSalary.StartSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.StartSalary"].Replace(".", "")) : 0,
                    };
        ...
        public ActionResult StaffInformation(int id)
        {
            ...
            model.ContractEndDate = staff.ContractEndDate;
            model.ContractFromDate = staff.ContractFromDate;
        ...
        public async Task<ActionResult> Update(StaffViewModel model, FormCollection form, HttpPostedFileBase Image)
        {
            ...
            try
            {
                ...
                if (await _staffRepository.Update(model.SingleStaff))
                {
                    ...
                    var salary = await _db.tbl_StaffSalary.FindAsync(model.SingleStaffSalary.Id);
                    if (salary != null)
                    {
                        // lương nhân viên
                        salary.BasicSalary = model.SingleStaffSalary.BasicSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.BasicSalary"].Replace(".", "")) : 0;
                        salary.SubsidySalary = model.SingleStaffSalary.SubsidySalary != null ? Convert.ToDecimal(form["SingleStaffSalary.SubsidySalary"].Replace(".", "")) : 0;
                        salary.AllowanceSalary = model.SingleStaffSalary.AllowanceSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.AllowanceSalary"].Replace(".", "")) : 0;
                        salary.CurSalary = model.SingleStaffSalary.CurSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.CurSalary"].Replace(".", "")) : 0;
                        salary.StartSalary = model.SingleStaffSalary.StartSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.StartSalary"].Replace(".", "")) : 0;
                        salary.Note = model.SingleStaffSalary.Note;
                    }
                    else
                    {
                        var salaryStaff = new tbl_StaffSalary()
                        {
                            StaffId = model.SingleStaff.Id,
                            BasicSalary = model.SingleStaffSalary.BasicSalary != null ? Convert.ToDecimal(form["SingleStaffSalary.BasicSalary"].Replace(".", "")) : 0,
                            SubsidySalary = model.SingleStaffSalary.SubsidySalary != null ? Convert.ToDecimal(form["SingleStaffSalary.SubsidySalary"].Replace(".", "")) : 0,
                        ...
                        _db.tbl_StaffSalary.Add(salaryStaff);
                    }
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_NhanVienTRAVELPLUS.xlsx");
                    ExportExcelTemplateCustomer(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-nhan-vien-TRAVELPLUS.xlsx";
